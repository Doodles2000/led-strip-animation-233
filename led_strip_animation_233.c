...
#include <sysparam.h>
#include <espressif/esp_wifi.h> // required to set hostname

// comment out the following when using LCM
//#include <espressif/esp_sta.h>
//#include "wifi.h"
//
// uncomment the following when using LCM
...
// comment the following when using LCM
...
homekit_characteristic_t name = HOMEKIT_CHARACTERISTIC_(NAME, "");

homekit_characteristic_t serial = HOMEKIT_CHARACTERISTIC_(SERIAL_NUMBER,"");
...
void hostname_check() {
    ...
    char *sysparam_hostname;
    uint8_t macaddr[6];
    sdk_wifi_get_macaddr(STATION_IF, macaddr);
    int calc_hostname_len = snprintf(NULL, 0, "neopixel-%02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);
    char *calc_hostname = malloc(calc_hostname_len + 1);
    snprintf(calc_hostname, calc_hostname_len + 1, "neopixel-%02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);

    status = sysparam_get_string("hostname", &sysparam_hostname);
        if (status != SYSPARAM_OK) {
            sysparam_set_string("hostname", calc_hostname);
        } else {
            if (calc_hostname != sysparam_hostname) {
                sysparam_set_string("hostname", calc_hostname);
                }
            }
            free (calc_hostname);
            free (sysparam_hostname);
...
void user_init(void) {
    ...
    uint8_t macaddr[6];
    sdk_wifi_get_macaddr(STATION_IF, macaddr);
    int name_len = snprintf(NULL, 0, "neopixel-%02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);
    char *name_value = malloc(name_len + 1);
    snprintf(name_value, name_len + 1, "neopixel-%02X%02X%02X", macaddr[3], macaddr[4], macaddr[5]);
    name.value = HOMEKIT_STRING(name_value);
    int serial_len = snprintf(NULL, 0, "%02X%02X%02X%02X%02X%02X", macaddr[0], macaddr[1], macaddr[2], macaddr[3], macaddr[4], macaddr[5]);
    char *serial_value = malloc(serial_len + 1);
    snprintf(serial_value, serial_len + 1, "%02X%02X%02X%02X%02X%02X", macaddr[0], macaddr[1], macaddr[2], macaddr[3], macaddr[4], macaddr[5]);
